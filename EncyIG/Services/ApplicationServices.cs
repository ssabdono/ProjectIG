...
namespace EncyIG.Services
{
    ...
    public class ApplicationServicesBase
    {
        ...
        public virtual JObject DefaultSettings
        {
            get
            {
                if (_defaultSettings == null)
                {
                    ...
                    EnsureJsonProperty(_defaultSettings, "appName", "Ency IG");
        ...
        {
            EncyIG.Services.ApplicationServices.Initialize();
        }
        ...
        public static void RegisterStandardMembershipAccounts()
        {
            ...
            if (css == null)
            	css = ConfigurationManager.ConnectionStrings["EncyIG"];
            ...
            if (Membership.GetUser("admin") == null)
            {
                ...
                admin = Membership.CreateUser("admin", "admin123%", "admin@EncyIG.com", "ASP.NET", "Code OnTime", true, out status);
                user = Membership.CreateUser("user", "user123%", "user@EncyIG.com", "ASP.NET", "Code OnTime", true, out status);
        ...
        public static void RegisterCssLinks(Page p)
        {
            foreach (Control c in p.Header.Controls)
            	if (c is HtmlLink)
                {
                    ...
                    if (l.ID == "EncyIGTheme")
                    	return;
                    if (l.Href.Contains("_Theme_Social.css"))
                    {
                        l.ID = "EncyIGTheme";
                        if (!(l.Href.Contains("?")))
    ...
    public class ApplicationSessionStateBase : SessionStateStoreProviderBase
    {
        ...
        public SqlText CreateSqlText(string sql)
        {
            ...
            cmd.Name = "EncyIG Session State Provider";
    ...
    public abstract class OAuthHandler
    {
        ...
        {
            if (ApplicationServices.IsSiteContentEnabled && (_config != null))
            {
                if (tokens.Length > 0)
                	_config.AccessToken = tokens[0];
                if ((tokens.Length == 2) && !(String.IsNullOrEmpty(tokens[1])))
                	_config.RefreshToken = tokens[1];
        ...
        public virtual void SetUserAvatar(MembershipUser user)
        {
            if (ApplicationServices.IsSiteContentEnabled)
            	try
                {
                    string url = GetUserImageUrl(user);
                    if (!(String.IsNullOrEmpty(url)))
                    {
                        WebRequest request = WebRequest.Create(url);
                        WebResponse response = request.GetResponse();
    ...
    public class OAuthHandlerFactoryBase
    {
        ...
        {
            if (ApplicationServices.IsSiteContentEnabled)
            	foreach (SiteContentFile file in ApplicationServices.Create().ReadSiteContent("sys/saas", "%"))
